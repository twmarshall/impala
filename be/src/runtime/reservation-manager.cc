...
void ReservationManager::Register(ReservationManager* rm, string name,
    RuntimeProfile* runtime_profile, MemTracker* mem_tracker,
    const TBackendResourceProfile resource_profile, const TDebugOptions debug_options) {
  rm->name_ = name;
  rm->runtime_profile_ = runtime_profile;
  rm->mem_tracker_ = mem_tracker;
  rm->resource_profile_ = resource_profile;
  rm->debug_options_ = debug_options;
}
