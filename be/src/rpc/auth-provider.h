...
class AuthProvider {
 public:
  ...
  /// Set the username, if available, of the user connected to 'underlying_transport' on
  /// 'connection_ptr'.
  virtual void SetConnectionUsername(
...
class NoAuthProvider : public AuthProvider {
 public:
  ...
  /// If there is no auth, then we don't have a username available, so do nothing.
  virtual void SetConnectionUsername(
      boost::shared_ptr<ThriftServer::ConnectionContext> connection_ptr,
      ThriftServer::TransportType underlying_transport_type,
      apache::thrift::transport::TTransport* underlying_transport) {}
