...
bool LdapCheckPass(const char* user, const char* pass, unsigned passlen) {
...
bool BasicAuth(ThriftServer::ConnectionContext* connection_context, const char* base64) {
  ...
  int64_t out_max;
  if (!Base64DecodeBufLen(base64, in_len, &out_max)) {
    LOG(ERROR) << "Failed to determine buffer length to decode base64 auth string.";
    return false;
  }
  char out[out_max];
  int64_t out_len;
  if (!Base64Decode(base64, in_len, out_max, out, &out_len)) {
    LOG(ERROR) << "Failed to decode base64 auth string.";
    return false;
  }
  char* colon = strchr(out, ':');
  if (colon == nullptr) {
    LOG(ERROR) << "Auth string must be in the form <username>:<password>";
    return false;
  }
  size_t user_len = colon - out;
  char user[user_len + 1];
  strncpy(user, out, user_len);
  user[user_len] = '\0';
  char* pass = colon + 1;
  size_t pass_len = out_len - user_len - 1;
  bool ret = LdapCheckPass(user, pass, pass_len);
  if (ret) {
    // Authenication was successful, so set the username on the connection.
    connection_context->username = user;
  }
  return ret;
}
...
void SecureAuthProvider::SetConnectionUsername(
    boost::shared_ptr<ThriftServer::ConnectionContext> connection_ptr,
    ThriftServer::TransportType underlying_transport_type,
  ...
  switch (underlying_transport_type) {
    case ThriftServer::BINARY: {
      TSaslServerTransport* sasl_transport =
          static_cast<TSaslServerTransport*>(underlying_transport);
    ...
    default:
      const string& err = Substitute("Bad transport type: $0", underlying_transport_type);
      LOG(ERROR) << err;
  }
}
