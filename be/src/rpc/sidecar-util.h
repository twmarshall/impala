...
static Status GetSidecar(int sidecar_idx, RPC* rpc, T* thrift_obj) {
...
// Serializes 'obj' and sets is as a sidecar on 'rpc_context' using a faststring, which
// transfers ownership of the sidecar memory to the RPC layer. This introduces another
// copy that wouldn't be necessary if using a Slice sidecar, but it's convenient for
// situations where it would be tricky to manually ensure that the sidecar's memory is
// released after the RPC has completed.
//
// If successful, the sidecar idx is returned in 'sidecar_idx'.
template <typename T>
Status SetFaststringSidecar(
    const T& obj, kudu::rpc::RpcContext* rpc_context, int* sidecar_idx) {
  ...
  RETURN_IF_ERROR(
      FromKuduStatus(rpc_context->AddOutboundSidecar(move(rpc_sidecar), sidecar_idx),
          "Failed to add sidecar"));
  return Status::OK();
}
