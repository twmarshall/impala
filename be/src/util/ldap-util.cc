...
DEFINE_string(ldap_bind_dn, "", "Distinguished name of the user to bind as when doing "
    "user or group searches. Only required if user or group filters are being used and "
    "the LDAP server is not configured to allow anonymous searches.");
DEFINE_string(ldap_bind_password, "", "Password for --ldap_bind_dn");
...
bool ImpalaLdap::LdapCheckFilters(std::string username) {
  ...
  LDAP* ld;
  bool success = Bind(FLAGS_ldap_bind_dn, FLAGS_ldap_bind_password.c_str(),
      FLAGS_ldap_bind_password.size(), &ld);
  if (!success) {
    return false;
  }
