...
void ImpalaHttpHandler::BackendsHandler(const Webserver::WebRequest& req,
  ...
  for (const auto& entry : membership_snapshot->current_backends) {
    ...
    Value mem_reserved(
        PrettyPrinter::PrintBytes(host_stats[address].mem_reserved).c_str(),
        document->GetAllocator());
    backend_obj.AddMember("mem_reserved", mem_reserved, document->GetAllocator());
    Value mem_admitted(
        PrettyPrinter::PrintBytes(host_stats[address].mem_admitted).c_str(),
        document->GetAllocator());
    ...
    backend_obj.AddMember(
        "num_admitted", host_stats[address].num_admitted, document->GetAllocator());
