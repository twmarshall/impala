...
void NonGroupingAggregator::Codegen(RuntimeState* state) {
  ...
  Status codegen_status = CodegenProcessBatch(codegen, prefetch_mode);
...
Status NonGroupingAggregator::CodegenProcessBatch(
  ...
  IRFunction::Type ir_fn = IRFunction::NON_GROUPING_AGG_PROCESS_BATCH_NO_GROUPING;
  ...
  void** codegened_fn_ptr = reinterpret_cast<void**>(&process_batch_no_grouping_fn_);
