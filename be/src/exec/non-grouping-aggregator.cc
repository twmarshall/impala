...
const char* NonGroupingAggregator::LLVM_CLASS_NAME =
    "class.impala::NonGroupingAggregator";
...
Status NonGroupingAggregator::GetRowsFromPartition(
...
Status NonGroupingAggregator::GetRowsStreaming(RuntimeState* state, RowBatch* out_batch) {
  DCHECK(!child_eos_);
  DCHECK(is_streaming_preagg_);
...
bool NonGroupingAggregator::ShouldExpandPreaggHashTables() const {
  int64_t ht_mem = 0;
  int64_t ht_rows = 0;
  for (int i = 0; i < PARTITION_FANOUT; ++i) {
...
void NonGroupingAggregator::CleanupHashTbl(
...
NonGroupingAggregator::Partition::~Partition() {
  DCHECK(is_closed);
}

Status NonGroupingAggregator::Partition::InitStreams() {
...
Status NonGroupingAggregator::Partition::InitHashTable(bool* got_memory) {
...
Status NonGroupingAggregator::Partition::SerializeStreamForSpilling() {
...
Status NonGroupingAggregator::Partition::Spill(bool more_aggregate_rows) {
  DCHECK(!parent->is_streaming_preagg_);
  DCHECK(!is_closed);
  DCHECK(!is_spilled());
...
void NonGroupingAggregator::Partition::Close(bool finalize_rows) {
  if (is_closed) return;
  is_closed = true;
...
Tuple* NonGroupingAggregator::ConstructIntermediateTuple(
...
int NonGroupingAggregator::GroupingExprsVarlenSize() {
  int varlen_size = 0;
...
void NonGroupingAggregator::CopyGroupingValues(
...
// TODO: codegen this function.
void NonGroupingAggregator::InitAggSlots(
...
void NonGroupingAggregator::UpdateTuple(
...
Tuple* NonGroupingAggregator::GetOutputTuple(
...
Status NonGroupingAggregator::AppendSpilledRow(
...
Status NonGroupingAggregator::CreateHashPartitions(int level, int single_partition_idx) {
...
Status NonGroupingAggregator::CheckAndResizeHashPartitions(
...
Status NonGroupingAggregator::NextPartition() {
...
Status NonGroupingAggregator::BuildSpilledPartition(Partition** built_partition) {
...
Status NonGroupingAggregator::RepartitionSpilledPartition() {
  DCHECK(!spilled_partitions_.empty());
  DCHECK(!is_streaming_preagg_);
  // Leave the partition in 'spilled_partitions_' to be closed if we hit an error.
...
template <bool AGGREGATED_ROWS>
Status NonGroupingAggregator::ProcessStream(BufferedTupleStream* input_stream) {
  DCHECK(!is_streaming_preagg_);
...
Status NonGroupingAggregator::SpillPartition(bool more_aggregate_rows) {
...
Status NonGroupingAggregator::MoveHashPartitions(int64_t num_input_rows) {
  DCHECK(!hash_partitions_.empty());
  stringstream ss;
...
void NonGroupingAggregator::PushSpilledPartition(Partition* partition) {
  DCHECK(partition->is_spilled());
...
void NonGroupingAggregator::ClosePartitions() {
...
// }
//
Status NonGroupingAggregator::CodegenUpdateSlot(LlvmCodeGen* codegen, int agg_fn_idx,
  ...
  if (tuple_struct == NULL) {
    return Status("NonGroupingAggregator::CodegenUpdateSlot(): failed to generate "
  ...
  *fn = codegen->FinalizeFunction(*fn);
  if (*fn == NULL) {
    return Status("NonGroupingAggregator::CodegenUpdateSlot(): codegen'd "
                  "UpdateSlot() function failed verification, see log");
  }
  return Status::OK();
}

Status NonGroupingAggregator::CodegenCallUda(LlvmCodeGen* codegen, LlvmBuilder* builder,
...
//
// define void @UpdateTuple(%"class.impala::NonGroupingAggregator"* %this_ptr,
...
Status NonGroupingAggregator::CodegenUpdateTuple(
  ...
  *fn = codegen->FinalizeFunction(*fn);
  if (*fn == NULL) {
    return Status("NonGroupingAggregator::CodegenUpdateTuple(): codegen'd "
...
Status NonGroupingAggregator::CodegenProcessBatchStreaming(
    LlvmCodeGen* codegen, TPrefetchMode::type prefetch_mode) {
  DCHECK(is_streaming_preagg_);
  ...
  if (process_batch_streaming_fn == NULL) {
    return Status("NonGroupingAggregator::CodegenProcessBatchStreaming(): codegen'd "
...
template Status NonGroupingAggregator::AppendSpilledRow<false>(Partition*, TupleRow*);
template Status NonGroupingAggregator::AppendSpilledRow<true>(Partition*, TupleRow*);
} // namespace impala
