...
void AggregationNode::GetSingletonOutput(RowBatch* row_batch) {
...
Status AggregationNode::GetRowsFromPartition(RuntimeState* state, RowBatch* row_batch) {
...
Status AggregationNode::GetRowsStreaming(RuntimeState* state, RowBatch* out_batch) {
  DCHECK(!child_eos_);
  DCHECK(is_streaming_preagg_);
...
bool AggregationNode::ShouldExpandPreaggHashTables() const {
  int64_t ht_mem = 0;
  int64_t ht_rows = 0;
  for (int i = 0; i < PARTITION_FANOUT; ++i) {
...
void AggregationNode::CleanupHashTbl(
...
AggregationNode::Partition::~Partition() {
  DCHECK(is_closed);
}

Status AggregationNode::Partition::InitStreams() {
...
Status AggregationNode::Partition::InitHashTable(bool* got_memory) {
...
Status AggregationNode::Partition::SerializeStreamForSpilling() {
...
Status AggregationNode::Partition::Spill(bool more_aggregate_rows) {
  DCHECK(!parent->is_streaming_preagg_);
  DCHECK(!is_closed);
  DCHECK(!is_spilled());
...
void AggregationNode::Partition::Close(bool finalize_rows) {
  if (is_closed) return;
  is_closed = true;
...
Tuple* AggregationNode::ConstructSingletonOutputTuple(
...
int AggregationNode::GroupingExprsVarlenSize() {
  int varlen_size = 0;
...
void AggregationNode::CopyGroupingValues(
...
// TODO: codegen this function.
void AggregationNode::InitAggSlots(
...
void AggregationNode::UpdateTuple(
...
Tuple* AggregationNode::GetOutputTuple(
...
Status AggregationNode::AppendSpilledRow(
...
string AggregationNode::DebugString(int indentation_level) const {
  stringstream ss;
...
Status AggregationNode::CreateHashPartitions(int level, int single_partition_idx) {
...
Status AggregationNode::CheckAndResizeHashPartitions(
...
Status AggregationNode::NextPartition() {
...
Status AggregationNode::BuildSpilledPartition(Partition** built_partition) {
...
Status AggregationNode::RepartitionSpilledPartition() {
  DCHECK(!spilled_partitions_.empty());
  DCHECK(!is_streaming_preagg_);
  // Leave the partition in 'spilled_partitions_' to be closed if we hit an error.
...
template <bool AGGREGATED_ROWS>
Status AggregationNode::ProcessStream(BufferedTupleStream* input_stream) {
  DCHECK(!is_streaming_preagg_);
...
Status AggregationNode::SpillPartition(bool more_aggregate_rows) {
...
Status AggregationNode::MoveHashPartitions(int64_t num_input_rows) {
  DCHECK(!hash_partitions_.empty());
  stringstream ss;
...
void AggregationNode::PushSpilledPartition(Partition* partition) {
  DCHECK(partition->is_spilled());
...
void AggregationNode::ClosePartitions() {
...
// }
//
Status AggregationNode::CodegenUpdateSlot(LlvmCodeGen* codegen, int agg_fn_idx,
  ...
  if (tuple_struct == NULL) {
    return Status("AggregationNode::CodegenUpdateSlot(): failed to generate "
  ...
  *fn = codegen->FinalizeFunction(*fn);
  if (*fn == NULL) {
    return Status("AggregationNode::CodegenUpdateSlot(): codegen'd "
                  "UpdateSlot() function failed verification, see log");
  }
  return Status::OK();
}

Status AggregationNode::CodegenCallUda(LlvmCodeGen* codegen, LlvmBuilder* builder,
...
// }
//
Status AggregationNode::CodegenUpdateTuple(LlvmCodeGen* codegen, llvm::Function** fn) {
  for (const SlotDescriptor* slot_desc : intermediate_tuple_desc_->slots()) {
    if (slot_desc->type().type == TYPE_CHAR) {
      return Status::Expected("AggregationNode::CodegenUpdateTuple(): cannot "
                              "codegen CHAR in aggregations");
    }
  }

  if (intermediate_tuple_desc_->GetLlvmStruct(codegen) == NULL) {
    return Status::Expected("AggregationNode::CodegenUpdateTuple(): failed to"
                            " generate intermediate tuple desc");
  }

  // Get the types to match the UpdateTuple signature
  llvm::PointerType* agg_node_ptr_type = codegen->GetStructPtrType<AggregationNode>();
  ...
  *fn = codegen->FinalizeFunction(*fn);
  if (*fn == NULL) {
    return Status("AggregationNode::CodegenUpdateTuple(): codegen'd "
                  "UpdateTuple() function failed verification, see log");
  }
  return Status::OK();
}

Status AggregationNode::CodegenProcessBatch(
  ...
  if (process_batch_fn == NULL) {
    return Status("AggregationNode::CodegenProcessBatch(): codegen'd "
...
Status AggregationNode::CodegenProcessBatchStreaming(
    LlvmCodeGen* codegen, TPrefetchMode::type prefetch_mode) {
  DCHECK(is_streaming_preagg_);
  ...
  if (process_batch_streaming_fn == NULL) {
    return Status("AggregationNode::CodegenProcessBatchStreaming(): codegen'd "
...
template Status AggregationNode::AppendSpilledRow<false>(Partition*, TupleRow*);
template Status AggregationNode::AppendSpilledRow<true>(Partition*, TupleRow*);
} // namespace impala
