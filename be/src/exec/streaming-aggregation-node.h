...
class StreamingAggregationNode : public ExecNode {
 ...
 private:
  ...
  typedef Status (*ProcessBatchNoGroupingFn)(StreamingAggregationNode*, RowBatch*);
  ...
  typedef Status (*ProcessBatchFn)(
      StreamingAggregationNode*, RowBatch*, TPrefetchMode::type, HashTableCtx*);
  /// Jitted ProcessBatch function pointer. Null if codegen is disabled.
  ProcessBatchFn process_batch_fn_;

  typedef Status (*ProcessBatchStreamingFn)(StreamingAggregationNode*, bool,
  ...
  struct Partition {
    Partition(StreamingAggregationNode* parent, int level, int idx)
    ...
    StreamingAggregationNode* parent;
