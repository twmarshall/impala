...
void GroupingAggregator::GetSingletonOutput(RowBatch* row_batch) {
...
Status GroupingAggregator::GetRowsStreaming(RuntimeState* state, RowBatch* out_batch) {
  DCHECK(!child_eos_);
  DCHECK(is_streaming_preagg_);
...
Tuple* GroupingAggregator::ConstructSingletonOutputTuple(
...
// TODO: codegen this function.
void GroupingAggregator::InitAggSlots(
...
void GroupingAggregator::UpdateTuple(
...
Tuple* GroupingAggregator::GetOutputTuple(
...
// }
//
Status GroupingAggregator::CodegenUpdateSlot(LlvmCodeGen* codegen, int agg_fn_idx,
  ...
  if (tuple_struct == NULL) {
    return Status("GroupingAggregator::CodegenUpdateSlot(): failed to generate "
  ...
  *fn = codegen->FinalizeFunction(*fn);
  if (*fn == NULL) {
    return Status("GroupingAggregator::CodegenUpdateSlot(): codegen'd "
                  "UpdateSlot() function failed verification, see log");
  }
  return Status::OK();
}

Status GroupingAggregator::CodegenCallUda(LlvmCodeGen* codegen, LlvmBuilder* builder,
...
//
// define void @UpdateTuple(%"class.impala::GroupingAggregator"* %this_ptr,
...
// }
//
Status GroupingAggregator::CodegenUpdateTuple(LlvmCodeGen* codegen, llvm::Function** fn) {
  for (const SlotDescriptor* slot_desc : intermediate_tuple_desc_->slots()) {
    if (slot_desc->type().type == TYPE_CHAR) {
      return Status::Expected("GroupingAggregator::CodegenUpdateTuple(): cannot "
                              "codegen CHAR in aggregations");
    }
  }

  if (intermediate_tuple_desc_->GetLlvmStruct(codegen) == NULL) {
    return Status::Expected("GroupingAggregator::CodegenUpdateTuple(): failed to"
                            " generate intermediate tuple desc");
  }

  // Get the types to match the UpdateTuple signature
  llvm::PointerType* agg_node_ptr_type = codegen->GetStructPtrType<GroupingAggregator>();
  ...
  *fn = codegen->FinalizeFunction(*fn);
  if (*fn == NULL) {
    return Status("GroupingAggregator::CodegenUpdateTuple(): codegen'd "
