...
public class AggregationNode extends PlanNode {
   ...
   * Takes the SUFFLE_DISTINCT_EXPRS query option into account.
   *
   * For single-class aggregations the returned exprs are typically the grouping exprs.
   * With SUFFLE_DISTINCT_EXPRS=true the distinct exprs are also included if this is the
   ...
   * and select the corresponding partition expr.
   ...
   *    constant sentinel exprs in the corresponding branch of the CASE.
   ...
   * The handling of SUFFLE_DISTINCT_EXPRS is analogous to the single-class case.
   *
   * Example:
   ...
   * 1. First class has distinct exprs a,b and second class has c
   ...
   *   WHEN 1 THEN 0 <-- sentinel constant integer
   * END
   */
  public List<Expr> getMergePartitionExprs(Analyzer analyzer) {
    ...
    if (aggInfos_.size() == 1) {
      ...
      if (aggPhase_.isFirstPhase() && hasGrouping() && !shuffleDistinctExprs) {
        groupingExprs = multiAggInfo_.getGroupingExprs();
      } else {
  ...
  public void computeNodeResourceProfile(TQueryOptions queryOptions) {
    ...
    if (aggInfos_.size() == 1) {
      ...
      // TODO: Determine node-level spillable buffer size. Does it even make sense?
  ...
  private ResourceProfile computeAggClassResourceProfile(
    ...
    ResourceProfile result = new ResourceProfileBuilder()
